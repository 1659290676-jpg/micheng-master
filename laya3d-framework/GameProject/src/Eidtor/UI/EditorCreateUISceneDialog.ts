...
/**
 * @ Author: XXL
 * @ Date: 2026-01-29 20:42
 * @ Description: 创建UI场景对话框
*/
@IEditor.regClass()
export default class EditorCreateUISceneDialog extends IEditor.Dialog {
    ...
    async create() {
        let _t = this;
        let panel = await gui.UIPackage.createWidget(EditorUtils.WIDGET_DIR + "CreateUIScene.widget");
        panel.text = "创建 UI 场景";
    ...
    private async createScene() {
        ...
        // 获取模版修改名称
        const templeteFile = path.join(EditorUtils.TEMPLET_DIR, "UITemplet.ls");
    ...
    public static async open(dir: string) {
        let panel = await Editor.getDialog(EditorCreateUISceneDialog);
        if (panel) {
            panel.hide();
            panel.show(null, dir);
        }

...
function registerAllFonts() {
    registerFont("layabox", "font/layabox.ttf");
}
...
var enableDcc2 = true;
//var appUrl = "http://192.168.16.208:7088/index.js"
...
const conchMarket = window.conchMarket;

if (gameInfo != null) {
    console.log("--------------- getDeviceInfo --------------------");
    conchMarket.getDeviceInfo((ret) => {
...
if (gameInfo != null && gameInfo.servHttp != null && gameInfo.servHttp.length > 0) {
    console.log("--------------- getAppData --------------------");

    conchMarket.getAppData({ http: gameInfo.servHttp, appId: gameInfo.appId + "", secretKey: gameInfo.secretKey, pf: gameInfo.pf, platform: gameInfo.platform }, (ret) => {
        if (ret) {
            try {
                console.log("--------------- getAppData result ---------------");
                console.log(ret);
                let d = JSON.parse(ret);
                if (d.code == 200 && d.data) {
                    let info = d.data;
                    let version = info.clientVer;
                    let cdnUrl = info.cdnUrl;
                    let resRoot = gameInfo.resRoot || "";
                    isUpdateAll = info.updateAll;
                    ...
                    if (cdnUrl) {
                        ...
                        cdnUrl += resRoot;
                        cdnUrl += version;
                        appUrl = cdnUrl + "index.js";
                        dccHead = cdnUrl + ".dcc/head.json";
                        dccUrl = cdnUrl + ".dcc/";
    ...
    console.log("No background address is configured; the game can be launched in single-player mode.")
    enterGame();
}
